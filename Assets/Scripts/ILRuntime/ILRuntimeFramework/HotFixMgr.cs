...
#else
        string abPath = Path.Combine(Application.dataPath, "../AssetBundles/PC/hotfixdll/");
        AssetBundle mainAB = AssetBundle.LoadFromFile(abPath + "dll");

        AssetBundleRequest requestDLL = mainAB.LoadAssetAsync("HotFix_Project.dll.bytes");
        AssetBundleRequest requestPDB = mainAB.LoadAssetAsync("HotFix_Project.pdb.bytes");
        yield return requestDLL;
        yield return requestPDB;

        if (requestDLL.isDone)
        {
            dll = BitConverter.GetBytes(requestDLL.asset);
        }

        if (requestPDB.isDone)
        {
            pdb = BitConverter.GetBytes(requestPDB.asset);
        }

        // string dllPath = Path.Combine(Application.dataPath, "AssetBundlePackage/HotFixDLL/HotFix_Project.dll.bytes");
...
// #else
//         string pdbPath = Path.Combine(Application.dataPath, "AssetBundlePackage/HotFixDLL/HotFix_Project.pdb.bytes");
