...
public class HotFixMgr : AutoGeneratedMonoSingleton<HotFixMgr>
{
    ...
    private IEnumerator LoadHotFixAssembly()
    {
        ...
        abPath = "";
#else
        abPath = System.IO.Path.Combine(Application.dataPath, "../AssetBundles/PC/hotfixdll/");
#endif
        // 加载AB包
        AssetBundle mainAB = AssetBundle.LoadFromFile(abPath + "dll");
        if (mainAB == null)
        {
            Debug.LogError($"加载AssetBundle热更包失败， Path：{abPath}");
            yield break;
        }
        // 加载 DLL 文件
        AssetBundleRequest requestDLL = mainAB.LoadAssetAsync<TextAsset>("HotFix_Project.dll.bytes");
        yield return requestDLL;
        byte[] dll = null;
        if (requestDLL.isDone)
        {
            dll = ((TextAsset) requestDLL.asset).bytes;
        }

        if (dll != null)
        {
            Debug.LogError("加载AssetBundle热更DLL失败，请确保已经通过VS打开Assets/Scripts/HotFix_Project~/HotFix_Project/HotFix_Project.sln编译过热更DLL");
        }

        m_fs = new System.IO.MemoryStream(dll);
#if DEBUG
        // 加载 PDB 文件
        AssetBundleRequest requestPDB = mainAB.LoadAssetAsync<TextAsset>("HotFix_Project.pdb.bytes");
        yield return requestPDB;
        byte[] pdb = null;
        if (requestPDB.isDone)
        {
            pdb = ((TextAsset) requestPDB.asset).bytes;
        }

        if (pdb == null)
        {
            Debug.LogError("加载AssetBundle热更PDB失败，请确保已经通过VS打开Assets/Scripts/HotFix_Project~/HotFix_Project/HotFix_Project.sln编译过热更DLL");
        }
