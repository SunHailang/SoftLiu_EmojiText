...
namespace Editor
{
    public class CreateSpriteAsset : EditorWindow
    {
        ...
        private static Code.SpriteAsset _spriteAsset;
        private static DrawSpriteAsset _drawSpriteAsset;

        [MenuItem("Tools/Create/Sprite Asset", false, 10)]
        static void main()
        {
        ...
        public static void OpenAsset(Code.SpriteAsset spriteAsset)
        {
            ...
            if (_sourceTex != null)
            {
                ...
                if (GUILayout.Button(_spriteAsset == null ? "Save" : "Save As"))
                {
                    ...
                    if (!string.IsNullOrEmpty(filePath))
                    {
                        _assetPath = filePath;
                        //创建序列化文件
                        _spriteAsset = ScriptableObject.CreateInstance<Code.SpriteAsset>();
                        _spriteAsset.TexSource = _sourceTex;
                        _spriteAsset.ListSpriteGroup = new List<Code.SpriteInforGroup>();
        ...
        //绘制信息的类
        private static void SetDrawSpriteAsset(Code.SpriteAsset spriteAsset)
        {
            //添加
            if (_drawSpriteAsset == null)
