...
namespace TooSimpleFramework.UI
{
    ...
    public class OutlineEx : BaseMeshEffect
    {
        ...
        private static UIVertex _SetNewPosAndUV(UIVertex pVertex, int pOutLineWidth, Vector2 pPosCenter, Vector2 pTriangleX, Vector2 pTriangleY, Vector2 pUVX, Vector2 pUVY,
            Vector4 pUVOrigin)
        {
            // Position
            ...
            Vector2 puvx = pUVX / pTriangleX.magnitude * posXOffset * (Vector2.Dot(pTriangleX, Vector2.right) > 0 ? 1 : -1);
            uv += new Vector4(puvx.x, puvx.y, 0, 0);
            Vector2 puvy = pUVY / pTriangleY.magnitude * posYOffset * (Vector2.Dot(pTriangleY, Vector2.up) > 0 ? 1 : -1);
            uv += new Vector4(puvy.x, puvy.y);
            pVertex.uv0 = uv; // 原始UV框
