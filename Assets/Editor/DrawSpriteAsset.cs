...
namespace Editor
{
    public class DrawSpriteAsset
    {
        ...
        private Code.SpriteAsset _spriteAsset;
        private bool _isSponsor;

        public DrawSpriteAsset(Code.SpriteAsset spriteAsset)
        {
        ...
        public void SetSpriteAsset(Code.SpriteAsset spriteAsset)
        {
            _spriteAsset = spriteAsset;
        }
        ...
        public void Draw()
        {
            if (_spriteAsset)
            {
                ...
                {
                    List<Code.SpriteInforGroup> inforGroups = _spriteAsset.ListSpriteGroup;
                    ...
                    for (int i = 0; i < inforGroups.Count; i++)
                    {
                        ...
                        if (_showIndex == i)
                        {
                            List<Code.SpriteInfor> spriteInfors = inforGroups[i].ListSpriteInfor;
        ...
        #region 内部函数
        //获取精灵信息
        private Code.SpriteInfor GetSpriteInfo(int index, int row, int column, Vector2 size, Vector2 texSize)
        {
            Code.SpriteInfor infor = Code.Utils.Pool<Code.SpriteInfor>.Get();
            infor.Id = index;
